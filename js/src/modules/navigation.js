...
(function ($) {
  ...
  (function () {
    ...
    menuToggle.on('click', function () {
      ...
      $('#nav-toggle-label').text(
        ...
        $('#nav-toggle-label').text() === imenet_screenReaderText.expand_toggle
      ...
      $(this).attr(
        'aria-label',
        $(this).attr('aria-label') === imenet_screenReaderText.expand_toggle
          ? imenet_screenReaderText.collapse_toggle
          : imenet_screenReaderText.expand_toggle,
      );
  ...
  // TODO: Rather fix the issue: https://github.com/digitoimistodude/imenet/issues/26
  // $(() => {
  //   if (window.innerWidth < responsivenav) {
  ...
  //     dropdownItem.attr('aria-label', imenet_screenReaderText.collapse);
  //     screenReaderSpan.text(imenet_screenReaderText.collapse);
  //   }
  // });

  // Add default dropdown-toggle label
  $('.dropdown-toggle').each(function () {
    $(this).attr('aria-label', `${imenet_screenReaderText.expand_for} ${$(this).prev().text()}`);
  });
  ...
  siteHeaderMenu.find('.dropdown-toggle').click(function (e) {
    if (enterPressed || window.innerWidth < responsivenav) {
      ...
      $(this).attr(
        'aria-label',
        $(this).attr('aria-label') === `${imenet_screenReaderText.collapse_for} ${$(this).prev().text()}`
          ? `${imenet_screenReaderText.expand_for} ${$(this).prev().text()}`
          : `${imenet_screenReaderText.collapse_for} ${$(this).prev().text()}`,
      );
    }
  });
...
// imenet code
// Close hamburger menu after clicking the menu item - uncomment for one-page sites
(function($) {
  container = document.getElementById("nav");
  if (!container) {
    return;
