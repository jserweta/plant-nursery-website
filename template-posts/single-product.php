					...
					<?php $product_terms = get_the_terms($post->ID, 'product_category');
                			if ($product_terms) : 
								foreach ($product_terms as $product_term) :
									...
									<h3 class="product-cat"><?php echo esc_html($term_name); ?></h3>
						<?php 	endforeach;
							endif;?>
									<h1 class="product-name"><?php echo esc_html(get_the_title()); ?></h1>
									<?php  if (!empty($subtitle) && isset($subtitle)): ?>
										<h4 class="title-latin"><?php echo esc_html($subtitle); ?></h4>
									<?php endif; ?>
					</div>

				<?php if (!empty($image_gallery) && isset($image_gallery)):?>
					<div class="swiper-container product__hero-swiper">
						<div class="swiper-wrapper">
							<?php foreach ($image_gallery as $index=>$img):?>
							<div class="swiper-slide">
								<img class="swiper-image" <?php awesome_acf_responsive_image($img['id'], 'full', '2000px'); ?> alt="<?= esc_html($img['alt']);?>">
							</div>
							<?php endforeach; ?>
						</div>
					...
					<div class="plant-parameters">
						...
						<div class="plant-parameters__list">
 						<?php foreach ($parameter as $p):
								...
								$value = $icon['value'];
								$label = $icon['label'];?>
							<div class="plant-parameters__item">
								...
								<div class="plant-parameters__value">
									<p><?php echo esc_html($param_value); ?></p>
								</div>
							</div>
						<?php endforeach;?>
						</div>
